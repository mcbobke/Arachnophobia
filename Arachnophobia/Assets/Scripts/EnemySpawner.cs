...
public class EnemySpawner : MonoBehaviour
{
    ...
    private GameObject[] SpawnList; //List of enemies to spawn
    ...
    private int maxIndex; //For accessing higher tier of enemies
    private int currentTier = 0; //Keeps track enemy tiers allowable

    //Boss behavior
    private int restartSpawn = 0;
    private bool bossActive = false;

    //Spawning Location
    private float xDistance = 6; //Max distance in x direction from center to spawn
    private float yDistance = 5; //Starting distance in sky
    ...
    private void Start()
    {
        ...
        maxIndex = SpawnAmount[0];
            // Last index of a created enemy in the game (e.g. how many enemies created, NOT spawned)
        ...
        for (int i = 0; i < MaxEnemies; i++)
        {
            ...
            {
                // If # enemies of current tier at peak, go to next tier
                currentTierCount = 0;
                eTier++;
            }
            currentTierCount++;
    ...
    private void SpawnEnemy()
    {
        ...
        {
            if (active >= maxIndex - 1)
            {
                break;
            } // Max number of enemies allowed to be active == last index of created enemies
            ...
            if (!SpawnList[spawnIndex].activeInHierarchy)
            {
                ...
                SpawnList[spawnIndex].transform.position = new Vector3(x_pos, y_pos, z_pos);
                    // Gives it a randomized position in x-space
                if (SpawnList[spawnIndex].tag == "SpiderTall")
                    // This if/for loop reenables disabled colliders in tall spiders that were previously spawned
                {
    ...
    private void EnemyTierProgression()
    {
        if (currentTier < EnemyTypes.Length - 1)
            maxIndex += SpawnAmount[currentTier++];
                // Increase the max index of spawnable enemies to be ANYTHING from 0 to the max of the next tier, e.g. 0-100 initially then 0-200
